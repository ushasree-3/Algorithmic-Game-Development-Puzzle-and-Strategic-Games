...
void generate_splits(const char *source, const char *dictionary[], int nwords, char buf[], void *data, void (*process_split)(char buf[], void *data))
{
    ...
    int word_lengths[nwords]; 
    int positions[nwords + 1]; 

    for (int i = 0; i < nwords; i++) {
        word_lengths[i] = strlen(dictionary[i]);
    }
    ...
    while (positions[0] >= 0) {
        ...
        for (int i = 0; i < nwords; i++) {
            if (positions[i] >= 0) {
                strncpy(buf + buf_index, source + positions[i], word_lengths[i]);
                buf_index += word_lengths[i];
                buf[buf_index] = ' '; 
                buf_index++;
            }
        }
        ...
        int j = nwords - 1;
        while (j >= 0 && positions[j] == source_len - word_lengths[j]) {
            j--;
        }
        ...
        for (int i = j + 1; i < nwords; i++) {
            positions[i] = positions[i - 1] + word_lengths[i - 1];
        }
    }
}
